...
// CONSTANTS
#define MAX_PAGES 100 // Integer in [MAX_PAGES,+inf)
...
void addition(float* matrix1,float* mtraix2,int num_rows,int num_cols);
...
int main(){
    ...
    float score_norm;
    do{
        // Store score column before operation
        ...
        // Repeat until score nrom is smaller than error
...
void init_link_matrix(int num_pages,float link_matrix[][num_pages]){
    ...
    for(int i=0;i<num_pages;i++){
        ...
        int num_links;
        do {
			printf("How many links does page %d have?\n", i + 1);
			scanf("%d", &num_links);
			if (num_links < 1)
		...
		for (int j = 0; j < num_links; j++) {
			int page_num;
			do {
				printf("%d. What page does page %d link?\n", j + 1, i + 1);
				scanf("%d", &page_num);
